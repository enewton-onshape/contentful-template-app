...
import { useCtfFeaturePageQuery } from './__generated/ctf-feature-page.generated';
import CtfFeaturePage from './ctf-feature-page';
...
const CtfFeaturePageGgl = ({ slug: slugFromProps }: Props) => {
  const slug = !slugFromProps || slugFromProps === '/' ? 'home' : slugFromProps;

  const { previewActive, locale } = useContentfulContext();

  const { isLoading, data } = useCtfFeaturePageQuery({
    slug,
    locale,
    preview: previewActive,
  });

  const page = useContentfulLiveUpdates(tryget(() => data?.featurePageCollection!.items[0]));
...
export default CtfFeaturePageGgl;
